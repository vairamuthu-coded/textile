...
import ProgressPopup from "../Custom/ProgressPopup";
import TabNav from "../component/TabNav";

import ImageUpload from "../Custom/ImageUploader";
...
const BarCodeGenerate = ({ title, subTitle }) => {
    ...
    const loadData = async () => {
      try {
        const [res1, res2, res3, res4, res5, res6, res7, res8] = await Promise.all([
          ...
          axios.get(`${GridLoadDetails}/${defaultDetails.Compcode}`),
  ...
  const TabIndexClick = async (inx) => {
    ...
    if (inx === 3) {
      const response = await axios.get(`${API_URL}/Reports/GenerateReportDetails`);
      alert(response.data);
  ...
  const GridLoad = async () => {
    try {
      ...
      var res = await axios.get(`${GridLoadDetails}/${defaultDetails.Compcode}`);
      setGrid(res.data);
    } catch (error) {
      setFetchError(error);
    } finally {
  ...
  const showPreview = (e) => {
    ...
    reader.onload = (x) => {
      ...
      setImage({
        ...images,
        imageFile: file,
        imagesrc: file.type === "application/pdf" ? defaultimage : result,
      });
  ...
  return (
    <form onSubmit={handleSubmit} style={{ cursor: isLoading ? "wait" : "default" }}>
      <ProgressPopup show={isLoading} progress={progress} colorValue={colorValue} />
      ...
      {userRights1.length >= 1 && (
        <div className="container-fluid animate-zoom p-1">
          <div className="row" style={{ display: `${userRights1[0].readonlys === "T" ? "block" : "none"}` }}>
            ...
            <div className="content-tabs boxShadow pt-1" style={{ height: "calc(100vh - 150px)", overflowY: "auto" }}>
                ...
                <div className="row animate-zoom">
                  <div className="col-md-11">
                    ...
                    <div className="row">
                      ...
                      <input type="checkbox" name="poCancel" className="col-md-1" checked={barValues.Pocancel} onChange={handleChange} onFocus={handleFocus} tabIndex={16} ref={(el) => (refs.current[16] = el)} onKeyDown={(e) => handleEnter(e, 16)} />

                      <Label className={`col-md-1`} labelName={"Total"}></Label>
                    </div>
                  </div>
                  <div className="col-md-1">
                    <ImageUpload imagesrc={images.imagesrc} onFileChange={showPreview} tabIndex={16} ref={(el) => (refs.current[14] = el)} />
                  </div>
            ...
            <div className={newButton === 3 ? "content active-content" : "content"}>
              <div className="row animate-zoom" style={{ height: "100%" }}>
                {pdfFile ? <iframe src={pdfFile} title="Invoice" style={{ width: "100%", height: "auto", border: "none" }} /> : <div className="text-muted">No PDF is selected yet.</div>}
              </div>
            </div>
              ...
              <div className="row animate-zoom">
                ...
                <div style={{ height: "446px", width: "100%", overflow: "auto" }}>
                  {/* {pdfFile && (
                        ...
                        //   <Viewer fileUrl={pdfFile} defaultLayoutPlugin={defaultLayoutPlugin}></Viewer>
                        // </Worker>
                      )} */}
                </div>
                <SocialMissing> {!pdfFile && <>No file is selected yet</>}</SocialMissing>
              </div>
            </div>
          </div>
        </div>
