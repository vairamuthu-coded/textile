...
const ProductionEntry = ({ title, subTitle }) => {
  ...
  const handleChange = async (e) => {
    ...
    if (name === "Compcode") {
      const res = await axios.get(`${ordertype_auto}/${value}/${title}/${TableName}`);
      if (res?.data?.[0]) {
        const item = res.data[0];

        setprodValues((prev) => ({
          ...prev,
          Asptblprolot1id: item.id || 0,
          Compcode1: item.gtcompmastid,
          Compcode: item.gtcompmastid,
          Compname: item.compname,
          Shortcode: item.shortcode,
          Prodno: `${year}-${item.shortcode}-${item.id}`,
          Proddate: today,
        }));
      }
    }
    ...
    if (name === "Pono") {
      ...
      const res2 = await axios.get(`${ponochange}/${prodValues.Compcode}/${value}`);
      if (res2?.data?.[0]) {
        ...
        setprodValues((prev) => ({
          ...prev,
          Pono: value,
          Asptblpurid: item.asptblpurid,
          Asptblpur1id: item.asptblpur1id,
          Orderno: item.orderno,
          Styleref: item.styleref,
          Stylename: item.asptblstymasid,
          barcodevalues1: `${item.minid}-${item.maxid}`,
          total: item.cnt,
          garmentimage: item.garmentimage ? `data:image/jpeg;base64,${item.garmentimage}` : null,
        }));
      }
    }
  };
  ...
  function ListData() {
    ...
    addRows1.forEach((row) => {
      const rowObject = {};

      addColumns1.forEach((cols, index) => {
        const cell = row[index];
        if (cell?.field) {
          rowObject[cell.field.trim()] = cell.value ?? 0;
        }
      });
  ...
  const ProductionEntry_Save = async () => {
    try {
      ...
      const { data } = await axios.post(insert_update, payload);
      ...
      const message = error.response?.data || error.response?.data?.message || error.message || "Something went wrong.";
  ...
  const BarcodeChanges = async (comcode, type, pono, bar) => {
    try {
      ...
      const data1 = response.data.map((datas, inx) => [
        ...
        { field: "BarCode", value: datas.barcode || 0 },
  ...
  const GridLoad_Check = async (id) => {
    ...
    setprodValues((prev) => ({
      ...
      garmentimage: res.data[0].garmentimage ? `data:image/jpeg;base64,${res.data[0].garmentimage}` : null,
    }));
    ...
    let rowData = resColor.data.map((row, index) => {
      return HeadersColumn1.map((col) => {
        switch (col.field) {
          ...
          case "Pcs":
            return { field: col.field, value: 1 };
                ...
                <div className="row animate-zoom">
                  <div className="col-md-11">
                    <div className="row">
                      ...
                      <input className="col-md-4" type="date" name="Proddate" style={{ color: `${colorValue}` }} value={prodValues.Proddate} onChange={handleChange} />
                    </div>
                    ...
                    <div className="row ">
                      ...
                      <Label className={`col-md-1`} forecolor={`${colorValue}`} labelName={"BarCode"}></Label>
                      <input
                        className="col-md-2"
                        type="text"
                        name="barcodevalues1"
                    ...
                    <div className="row py-1">
                      ...
                      <label className="col-md-2 hdr">{prodValues.total === undefined ? "Total  :  0" : "Total  : " + prodValues.total}</label>
                    </div>
                  </div>

                  <ImageCard src={prodValues.garmentimage} alt="garment" borderColor={colorValue} className="col-md-1 p-2" maxHeight="100px" maxWidth="150px" />
                  ...
                  <div className="col-md-12">
                    ...
                    />
                    <strong className="p-2" style={{ backgroundColor: colorValue, color: foreValue }}>
                      {totalcounts}
                    </strong>
                  </div>
                </div>
