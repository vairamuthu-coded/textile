...
const CheckingEntry = ({ title, subTitle }) => {
  ...
  const CheckingEntry_Search = async (e) => {
    try {
    } catch (err) {
      toast.error(err);
    }
  ...
  useEffect(() => {
    async function GetApi() {
      ...
      const [res, res1, res2, res3, res4] = await Promise.all([res_rihgts, res_com, res_grid, res_rem]);
              ...
              <div className={newButton === 1 ? "content active-content" : "content"}>
                <div
                  ...
                  style={{
                    ...
                    background: `linear-gradient(90deg, lightgray 0%, rgba(255,255,255,0.3) 100%)`,
                  }}
                >
                  <Label className={`col-md-1`} name="Type" forecolor={`${foreValue}`} labelName={"Process "}></Label>
