...
const CheckingEntry = ({ title,subTitle }) => {
  const { API_URL, newButton, inputref, handleSubmit, tabindex,  currentPage, setCurrentPage,
    CountryParam, searchLable1, searchLable2, searchLable3, bgValue,
    setSearchLable1, setSearchLable2, setSearchLable3,defaultDetails,
    sequenceTable, colorValue,loginCompCode,loginUser,
   foreValue ,checkValues, setCheckValues,addRows3, setAddRows3,
    sorting, setSorting, setNewButton, color1, addColumns3, setAddColumns3,  HeadersColumn3,chkpo, setChkPo } = useContext(DataContext);
  ...
  let ITEM_PER_PAGE = 20;  const [totalcounts, setTotalcounts] = useState(0);
  ...
  const heights = "430px"; let validorder = "";
  ...
  async function GetApi() {
   const res_rihgts=axios.get(`${userrightsMenuCheck}/${defaultDetails.Compcode}/${defaultDetails.User}/${title}`).catch(error => { throw{"Source":setFetchError(error)}});
   const res_com=axios.get(`${compcodeparam}`).catch(error => { throw{"Source":setFetchError(error)}});
    const res_grid= axios.get(`${GridLoadDetails}`).catch(error => {  throw{"Source":setFetchError(error)} });
    const res_rem=  axios.get(`${Remarks}`).catch(error => {  throw{"Source":setFetchError(error)} });
...
const handleChange = async (e) => {
  ...
  const GridLoad_Check = async (id) => {
    ...
    setAddRows3([]); setAddColumns3([]); var colorlength = 0;
    setCheckValues([])

    var res = await axios.get(`${GridLoadDetails}/${id.asptblchkid}/${id.compcode}`);
    if (res.data.length === 0) { toast.error("Invalid Data");return } else {
     const header = res.data[0];
      ...
      await axios.get(`${PonoDetails}/${header.compcode}/${header.issuetype}`)
        .then((res1) => {
          setChkPo(res1.data)
     ...
     if (data.length > 1) {
       var j = 0;
        await axios.get(`${GridLoadColor}/${header.asptblchkid}/${header.compcode}`)
          .then(async (res1) => {
            setAddColumns3(data);
  ...
  const CheckingEntry_News = async () => {
    setNewButton(1); data = []; setCheckValues([])
    setActive(false);setQrbarValues([]);setProcessItems([])
      const ponoPromise = await axios.get(`${GridLoadDetails}`);       
         setGrid(ponoPromise.data)    
    setAddRows3([]); setAddColumns3(HeadersColumn3); RowsData = []
  }
...
//p.Pono + "," + p.Asptblchkid + "," + p.Compcode + "," + p.Finyear + "," + p.Asptblchk1id
    ...
    return [{
      Pono:Pono,Asptblchkid:Asptblchkid,Compcode:Compcode,
      Finyear:Finyear,Asptblchk1id:Asptblchk1id
  }]}
          ...
          const response=res.data;
           ...
           const param = await ParamsListData(response);
              if (!users || !param) {
               ...
               var response1;let j=0;
              for (let i = 0; i < userss.length; i++) {
                response1="";j+=userss[i].length;
...
<div className='row ' style={{ display: `${userRights1[0].readonlys === "T" ? "block" : "none"}` }}>
  <ul className="boxShadow" style={{ display: "flex", justifyContent: "right",margin:'0 0 5px 0'  }}>
             <li > <button  className={newButton === 1  ? "tabs active-tabs" : "tabs" } style={{display:`${userRights1[0].news}`==='T' ? "block" : "none",backgroundColor:`${colorValue}`}} onClick={() => CheckingEntry_News()}    >News</button></li>
             <li > <button  className={newButton === 2 ? "tabs active-tabs" : "tabs"} style={{display:`${userRights1[0].saves}` === "T" ? "block" : "none",backgroundColor:`${colorValue}`,}}  onClick={() => CheckingEntry_Save()}    >Save</button></li>
             <li > <button  className={newButton === 3 ? "tabs active-tabs" : "tabs"} style={{display:`${userRights1[0].deletes}` === "T" ? "block" : "none",backgroundColor:`${colorValue}`}}  onClick={(e) => CheckingEntry_Delete(checkValues.asptblchkid)}  >Delete</button></li>
             <li > <button  className={newButton === 4 ? "tabs active-tabs" : "tabs"} style={{display:`${userRights1[0].search}` === "T" ? "block" : "none",backgroundColor:`${colorValue}`}}  onClick={() => CheckingEntry_News()}  > Search </button></li>
             <li > <button  className={newButton === 5 ? "tabs active-tabs" : "tabs"} style={{display:`${userRights1[0].prints}` === "T" ? "block" : "none",backgroundColor:`${colorValue}`}}  onClick={() => CheckingEntry_News()} >Prints</button></li>
             <li > <button  className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{display:`${userRights1[0].readonlys}` === "T" ? "none" : "none",backgroundColor:`${colorValue}`}} onClick={() => CheckingEntry_News()} >Readonlys</button></li>
             <li > <button  className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{display:`${userRights1[0].treebutton}` === "T" ? "block" : "none",backgroundColor:`${colorValue}`}}  onClick={(e) => CheckingEntry_News()} >TreeButton</button></li>
             <li > <button  className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{display:`${userRights1[0].globalsearch}` === "T" ? "block" : "none",backgroundColor:`${colorValue}`}}  onClick={() => CheckingEntry_News()} > Globalsearch </button></li>
             <li > <button  className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{display:`${userRights1[0].login}` === "T" ? "block" : "none",backgroundColor:`${colorValue}`}}  onClick={() => CheckingEntry_News()} >Login</button></li>
             <li > <button  className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{display:`${userRights1[0].changepassword}` === "T" ? "block" : "none",backgroundColor:`${colorValue}`}}  onClick={() => CheckingEntry_News()} >Changepassword</button></li>
             <li > <button  className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{display:`${userRights1[0].changeskin}` === "T" ? "block" : "none",backgroundColor:`${colorValue}`}}  onClick={(e) => CheckingEntry_News()} >Changeskin</button></li>
             <li > <button  className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{display:`${userRights1[0].contact}` === "T" ? "block" : "none",backgroundColor:`${colorValue}`}} onClick={() => CheckingEntry_News()} > Contact </button></li>
             <li > <button  className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{display:`${userRights1[0].pdf}` === "T" ? "block" : "none",backgroundColor:`${colorValue}`}}  onClick={(e) => CheckingEntry_News()} >Pdf</button></li>
             <li > <button  className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{display:`${userRights1[0].imports}` === "T" ? "block" : "none",backgroundColor:`${colorValue}`}}  onClick={() => CheckingEntry_News()} > Import </button></li>
             <li > <button  className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{display:`${userRights1[0].download}` === "T" ? "block" : "none",backgroundColor:`${colorValue}`}}  onClick={() => CheckingEntry_News()} > Download </button></li>

     </ul>


             <ul className='' style={{backgroundColor:`${colorValue}`}}>
            <li>  <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} onClick={() => TabIndexClick(1)} style={{ backgroundColor:`${colorValue}`,width:'100%' }}>{title}  </button></li> 
            <li> <button className={newButton === 2 ? "tabs active-tabs" : "tabs"} onClick={() => TabIndexClick(2)} style={{ backgroundColor:`${colorValue}`,}} > {subTitle} </button></li> 

            </ul>
            <div className='content-tabs'>
              <div className={newButton === 1 ? "content active-content" : "content"}>
                <div className='row'  >
                   <h6 className='d-flex justify-content-center p-1' style={{backgroundColor:`${colorValue}`}}  >
                    <Label className={`col-md-2 ps-2`} name="CompCode" forecolor={`${foreValue}`} labelName={"CompCode "} visible={true}  ></Label>
                    <select className='col-md-1' name="Compcode" style={{ color: `${colorValue}` }}
                      value={checkValues.Compcode || ""} onChange={handleChange} >
                           <option></option>
                        {
                    ...
                    <select className='col-md-3' name="IssueType" ref={inputref}  tabIndex="1" style={{ color: `${colorValue}` }}
                      value={checkValues.IssueType || ""} onChange={handleChange} >
                      <option></option>
                    {                                                        
                          ProcessType.map((result, index) => (<option key={index} value={result.processname}>
                                {result.processname}
                            </option>))
                              }
                    </select>
                   <Label className={`col-md-1 ps-2`} name="Pono" forecolor={`${foreValue}`} labelName={"Pono "} visible={true}  ></Label>
                    <select className='col-md-3' name="Pono" ref={inputref} tabIndex="2" style={{ color: `${colorValue}` }}
                      value={checkValues.Pono || ""} onChange={handleChange} >
                      <option></option>{                       
                        chkpo.map((result, index) => (<option key={index} value={result.pono}>
                          {result.pono}
                        </option>))
                      }
                    </select>
                  ...
                  <div className='row py-1'>
                    <Input type={'text'} className1='col-md-1' className='col-md-1' id='Asptblchkid' name='Asptblchkid' placeholder='' value={checkValues.Asptblchkid || ""} onChange={handleChange} barValues={checkValues} setBarValues={setCheckValues} name1={"DefectID"} stylecolor={`${colorValue}`} visible={true} tabIndex={3} disabled='true' />
                    <Input type={'text'} className1='col-md-1' className='col-md-1' id='Asptblchk1id' name='Asptblchk1id' placeholder='' value={checkValues.Asptblchk1id || ""} onChange={handleChange} barValues={checkValues} setBarValues={setCheckValues} name1={"Defect1ID"} stylecolor={colorValue} visible={true} tabIndex={4} disabled='true' />
                    <Input type={'text'} className1='col-md-1' className='col-md-1' id='Shortcode' name='Shortcode' placeholder='' value={checkValues.Shortcode || ""} onChange={handleChange} barValues={checkValues} setBarValues={setCheckValues} name1={"ShortCode"} stylecolor={colorValue} visible={true} tabIndex={5} disabled='true' />
                    <Input type="date" className1='col-md-1' className='col-md-2' id='Wdate' name='Wdate' placeholder='' value={checkValues.Wdate} onChange={handleChange} barValues={checkValues} setBarValues={setCheckValues} name1={"Date"} stylecolor={`${colorValue}`} visible={true} tabIndex={6} />
                    <Input type={'text'} className1='col-md-1' className='col-md-2' id='Checkno' name='Checkno' placeholder='' value={checkValues.Checkno || ""} onChange={handleChange} barValues={checkValues} setBarValues={setCheckValues} name1={"CutPanelno"} stylecolor={colorValue} visible={true} tabIndex={7} disabled='true' />
                  </div>
                  <div className='row'>
                     <Label className={`col-md-1`} labelName={"Processname"} forecolor={`${colorValue}`} visible={true} ></Label> 
                   <select className='col-md-2' name="Processname" ref={inputref} tabIndex="6" style={{ color: `${colorValue}` }}
                      onChange={handleChange}  value={checkValues.Processname || ''} >   
                                {                                                        
                    ...
                    <select className='col-md-2' name="Remarks" tabIndex="7" ref={inputref}
                      ...
                      <option>{checkValues.IssueType || ""}</option>

                    </select>
                    <Input type={'text'} className1='col-md-1' className='col-md-2' id='OrderNo' name='OrderNo' placeholder='' value={checkValues.Orderno || ""} onChange={handleChange} barValues={checkValues} setBarValues={setCheckValues} name1={"Erp-OrderNo"} stylecolor={`${colorValue}`} visible={true} tabIndex={8} disabled='true' />
                    <Input type={'text'} className1='col-md-1' className='col-md-2' id='Styleref' name='Styleref' placeholder='' value={checkValues.Styleref || ""} onChange={handleChange} barValues={checkValues} setBarValues={setCheckValues} name1={"Styleref"} stylecolor={colorValue} visible={true} tabIndex={9} disabled='true' />
                  ...
                  <div className='row pt-1'>
                    ...
                    <select className='col-md-2' name="Notes" tabIndex="11" ref={inputref}
                      value={checkValues.Notes || ""} onChange={handleChange} >
                      <option></option>
                      <option value={"PCS RATE"} >PCS RATE</option>
                      <option value={" MONTHLY"} > MONTHLY</option>
                    </select>
                    <Input type={'text'} className1='col-md-1' className='col-md-5' id='Barcode' name='Barcode' placeholder='' value={checkValues.Barcode || ""} onChange={handleChange} axioss1={ponochange} barValues={checkValues} setBarValues={setCheckValues} name1={"BarCode"} stylecolor={colorValue} visible={true} tabIndex={11} />
                  </div>
                </div>
                <div className='row pt-1'>
                  <label className="col-md-2" color={`${colorValue}`} style={{ fontSize: "20px", fontWeight: "bold", ali:"center", }}   >BarCode SCan</label>
                  ...
                  <button type='submit' className='col-md-1' onClick={() => BarcodeChanges(checkValues.Compcode, checkValues.IssueType, checkValues.Pono, Qrbarvalues)} style={{ display: userRights1[0].news === "T" ? "block" : "none",backgroundColor:`${colorValue}`,color:`${foreValue}` }}  >ADD</button>
