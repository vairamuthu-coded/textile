...
const Search = ({ colorValue, stylecolor, searchs, setsearchs, SearchLable1, SearchLable2, SearchLable3, searchCompCode, ChangeValues, handleChange, searchUserName }) => {
  return (
    <div className="container-fluid border-1 ">
      <div className="row p-2" style={{ background: `${colorValue}` }}>
        {SearchLable1 !== "" ? <Label className={`col-md-2`} forecolor={stylecolor} labelName={SearchLable1}></Label> : ""}
        {SearchLable1 !== "" ? <input className="col-md-3 p-1" type="text" name="SearchItem" placeholder="Search Items" aria-label="SearchItem" value={searchs} onChange={(e) => setsearchs(e.target.value)} /> : ""}
