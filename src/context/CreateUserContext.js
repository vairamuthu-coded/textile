...
export const DataProvider=({children,API_URL,colorValue,bgValue, localServerCart,
  defaultDetails,header_items,menuheader,headerfilterdata,sidebarData,foreValue,
  sidebar,setSidebar,showSidebar,  headerdrop,setHeaderDrop,urls,
  setSelectedTitle,selectedTitle,mode, setMode
 })=>{
...
let tablecheck=false;let inputref=useRef();
...
//     for(let i=1;i<dataFromWeb.length+1;i++){
//     cart[i]=0;
//     }
//     return cart;
//     }
   
//     const addItemss=(itemid)=>{          
        
//      //let  newitem= dataFromWeb.map((item)=>{item.id === itemid})
//       alert(itemid)
...
//const contextValue={cartItem,addToCart,removeFromCart,updateToCart};
...
//End Cart 
 ...
 // const [userRightValues, setUserRightValues] = useState([]);
  const [sequenceTable, setSequenceTable] = useState("asptblautogeneratemas");
   ...
   // const heading = API_URL + "/UserMaster/Headings";
   // const ScreenName = API_URL + "/UserMaster/ScreenName";
   // const t1 = API_URL + "/CompanyMaster/GridLoad";
    //const t2 = API_URL + "/UserRights/UserRightsCheck";
    //const treeviewrights = API_URL + "/UserMaster/ScreenNameHeadings";
    //const treeviewrightss = API_URL + "/UserMaster/ScreenNameHeadingss";
...
const handlepage = async (selectedTitle) => {
  ...
  function tabpageClick(element, index) {
    ...
    setHeaderSearch(""); setHeaderDrop(false);
     setSidebar(false)
    navigate(`/${lists}`);
  }
...
function handleClose(index, name) {
  ...
  // switch (name) {
  //   case "CountryMaster":     setCountryValues([]); return;
  //   case "StateMaster":       setStateValues([]);    return;
  //   case "CityMaster":        setCityValues([]);   return;
  //   case "CompanyMaster":     setCompanyValues([]);    return;
  //   case "BarcodeGenerate":   setBarValues([]);    setAddRows([]);   return;
  //   case "ProductionEntry":   setprodValues([]);  setAddRows1([]);  return;
  //   case "DefectEntry":       setDefectValues([]);   return;
  //   case "CheckingEntry":     setCheckValues([]);   setAddRows3([]);   return;
  //   case "BuyerMaster":       setBuyerValues([]);   return;
  //   case "AutoGenerateMaster":setAutoValues([]); return;

  //   default:      return;
  // }
  if (name === "CountryMaster") { setCountryValues([]); return; }
  if (name === "StateMaster") { setStateValues([]); return; }
  if (name === "CityMaster") { setCityValues([]); return; }
  if (name === "CompanyMaster") { setCompanyValues([]); return; }
  if (name === "BarcodeGenerate") { setBarValues([]); setAddRows([]); return; }
  if (name === "ProductionEntry") { setprodValues([]); setAddRows1([]); return; }
  if (name === "DefectEntry") { setDefectValues([]); return; }
  if (name === "CheckingEntry") { setCheckValues([]); setAddRows3([]); return; }
  if (name === "BuyerMaster") { setBuyerValues([]); return; }
  if (name === "AutoGenerateMaster") { setAutoValues([]); return; }
    ...
    return(     
        ...
        {{
          ...
          handleClose,tabpageClick,lastindex,lastindex1,header_search,setHeaderSearch,
          handlepage,sidebarData,sidebar, setSidebar,showSidebar,bgValue,
            localServerCart, menuheader,headerfilterdata,headerdrop,setHeaderDrop,
            error,setError,foreValue, mode, setMode,colorValue,
          po, setPo,defpo, setDefPo,propo, setProPo,chkpo, setChkPo,statuspo, setStatusPo,
            // dataFromWeb,cartItem,addToCart,removeFromCart,
            // updateToCart,addItemss,
            selectText,setSelectedText,userRights,setUserRights,treeviewdata,counts,setCounts,
            searchLable1,searchLable2,searchLable3,checkall,setCheckAll,checkchild,setCheckchild,
            setSearchLable1,setSearchLable2,setSearchLable3,
             newButton, setNewButton,   tablecheck,  
             handleSubmit,urls,API_URL,IntialzeState,isloading,setIsLoading,
             defaultDetails,sequenceTable,setAddRows2,header_items,
              countryValues, setCountryValues,
              stateValues,setStateValues,
             cityValues,setCityValues,inputref,
             companyValues, setCompanyValues,
             stateItems, setStateItems,
             countryItems, setCountryItems,processValues, setProcessValues, 
             cityStateData,setCityStateData,cityCountryData,setCityCountryData,
             color1,
             buyerValues, setBuyerValues,
             sizeValues, setSizeValues,sizeGroup,setSizeGroup,colorValues, setColorValues,
             employeeValues, setEmployeeValues,styleItemValues, setStyleItemValues,
             styleCatValues,setStyleCatValues,styleGroupValues, setStyleGroupValues,fabtype,setFabType,
             yarnBlend,setYarnBlend,fab,setFab,
            barValues, setBarValues,prodValues,setprodValues,
            currentPage,setCurrentPage,sorting,setSorting,ITEM_PER_PAGE,
             addRows, setAddRows,
              addColumns, setAddColumns,HeadersColumn,addColumns1, setAddColumns1,HeadersColumn1,
              HeadersColumn2,
              autoValues, setAutoValues,selectedValue, setSelectedValue,selectedTitle, setSelectedTitle,
             addRows1, setAddRows1, defectValues,setDefectValues,
             addColumns2, setAddColumns2,addRows2, setAddRows2,checkValues,setCheckValues,addRows3, setAddRows3,
             addColumns3, setAddColumns3,HeadersColumn3,
        }}>
            {children}
