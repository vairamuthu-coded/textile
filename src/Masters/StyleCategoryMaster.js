...
const StyleCategoryMaster = ({title,subTitle,colorValue}) => {   
  const {API_URL,newButton,handleSubmit, tabindex, totalItems,setTotalItems,currentPage,setCurrentPage,CountryParam,
    sorting,setSorting, setNewButton,styleCatValues,setStyleCatValues}=useContext(DataContext)
 let ITEM_PER_PAGE=15;
 const insert_update=API_URL+"/StyleCategoryMasters/PostStyleCategoryMaster";
 const stylecategoryparam=API_URL+"/StyleCategoryMasters/Getasptblstycatmas";
 const deleteData=API_URL+"/StyleCategoryMasters/DeleteStyleCategoryMaster";
 const getData=API_URL+"/StyleCategoryMasters/GetStyleCategoryMaster";
...
// if(validcheck==true){
    ...
    try
    {    
      const  CountryData={asptblstycatmasid:styleCatValues.asptblstycatmasid> 0  ? styleCatValues.asptblstycatmasid : 0,stylecategory:styleCatValues.stylecategory,Active:active===true ? "T" : "F"}; 
...
const StyleCategoryMaster_Delete= async()=>{    
  try{
    if(styleCatValues.stylecategory === ''){alert(`Empty Not Allowed`);return;}
    if(styleCatValues.asptblstycatmasid>=1){
    const asptblstycatmasid=styleCatValues.asptblstycatmasid;
    await axios.delete(`${deleteData}/${asptblstycatmasid}`)
    .then((respose)=>{    
     if(respose.data.asptblstycatmasid > 0){    
  ...
  const StyleCategoryMasterNew=()=>{   setNewButton(1);    setActive(false) ;
...
<div className='col-md-12' style={{textAlign:"right"}}>
 <div style={{background :"var(--bs-header)"}}>
   <ul >    
       <li> <button type='submit'  onClick={()=>StyleCategoryMasterNew()} style={{backgroundColor:`${colorValue}`}}>News</button></li>
       <li> <button type='submit' onClick={()=>StyleCategoryMaster_Save()} style={{backgroundColor:`${colorValue}`}}>Save</button></li>          
       <li> <button type='submit' onClick={(e)=>StyleCategoryMaster_Delete()} style={{backgroundColor:`${colorValue}`}}>Delete</button></li>
       <li> <button type='submit'  onClick={()=>StyleCategoryMasterNew()} style={{backgroundColor:`${colorValue}`}}> Search </button></li>
    </ul>
 </div>
 </div>
