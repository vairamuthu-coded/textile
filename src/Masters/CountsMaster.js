...
const CountsMaster = ({ title }) => {
  ...
  const { newButton, setNewButton, userRights, setUserRights,
    currentPage, setCurrentPage, API_URL, colorValue, defaultDetails,
    counts, setCounts, handlepage,
      ...
      <div className='container-fluid animate-zoom' >
        <div className='row'>
          ...
          <ul style={{ backgroundColor:`${colorValue}`,  borderBottom:"2px solid white", textAlign: "right" }}>
            <li> <button type='submit' style={{ backgroundColor: `${color1[0]}` }}>News</button></li>
            <li> <button type='submit' onClick={() => CountsMaster_Save()} style={{ backgroundColor: `${color1[2]}` }}>Save</button></li>
          </ul>
        ...
        <ul className='row'>
          {
             ...
             names.map((fstate) => (
              ...
              <li className='col-md-6'  key={fstate.id} onClick={() =>dispacthForm({ type: ACTION.ADD_EDIT, payload:fstate.id })} >{fstate.id}-{fstate.counts}-{fstate.email}-{fstate.age}
                <button onClick={() =>  dispacthForm({ type: ACTION.ADD_DELETE, payload:fstate.id })} style={{ color: 'red' }} >Detete</button>
                <button onClick={() => dispacthForm({ type: ACTION.ADD_UPDATE, payload:{id:fstate.id }})} style={{ color: 'green' }}>Update</button>
              </li>
             ))
