...
const FabricTypeMaster = ({title,subTitle,colorValue}) => {   
  const {API_URL,newButton,handleSubmit, tabindex, totalItems,setTotalItems,currentPage,setCurrentPage,CountryParam,
    fabtype,setFabType,searchLable1,searchLable2,searchLable3,
...
setNewButton(1);
 ...
 useEffect(()=>{   
  ...
  .then((res) => {setItems(res.data); fabtype.asptblfabrictypemasid='0';})
...
try
{    
  const  CountryData={asptblfabrictypemasid:fabtype.asptblfabrictypemasid> 0  ? fabtype.asptblfabrictypemasid : 0,
    fabrictype:fabtype.fabrictype,active:active===true ? "T" : "F"}; 
...
const FabricTypeMaster_Delete= async()=>{    
  try{
    if(fabtype.fabrictype === ''){alert(`Empty Not Allowed`);return;}
    if(fabtype.asptblfabrictypemasid>=1){
    const asptblfabrictypemasid=fabtype.asptblfabrictypemasid;
    await axios.delete(`${deleteData}/${asptblfabrictypemasid}`)
    .then((respose)=>{
...
const inputref=useRef();
  ...
  const FabricTypeMaster_New=()=>{   setNewButton(1);      setActive(false) ;
...
<div className='col-md-12' style={{textAlign:"right"}}>
 <div style={{background :"var(--bs-header)"}}>
   <ul >    
       <li> <button type='submit'  onClick={()=>FabricTypeMaster_New()} style={{backgroundColor:`${color1[0]}`}}>News</button></li>
       <li> <button type='submit' onClick={()=>FabricTypeMaster_Save()} style={{backgroundColor:`${color1[1]}`}}>Save</button></li>          
       <li> <button type='submit' onClick={(e)=>FabricTypeMaster_Delete()} style={{backgroundColor:`${color1[2]}`}}>Delete</button></li>
       <li> <button type='submit'  onClick={()=>FabricTypeMaster_New()} style={{backgroundColor:`${color1[3]}`}}> Search </button></li>
    </ul>
 </div>
 </div>
...
<Search colorValue={colorValue} searchs={search} setsearchs={setSearch}
              ...
              handleChange={handleChange} ChangeValues={fabtype}
