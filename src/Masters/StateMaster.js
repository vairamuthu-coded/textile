...
const StateMaster = ({ title, subTitle }) => {
  ...
  useEffect(() => {
    const fetchMyAPI = async () => {
      try {
        const [rightsRes, stateRes, countryRes] = await Promise.all([axios.get(`${userrightsMenuCheck}/${defaultDetails.Compcode}/${defaultDetails.User}/${title}`), axios.get(`${API_URL}${StateParam}`), axios.get(`${API_URL}${CountryParam}`)]);
  ...
  const StateMasterCheck = async (id) => {
    try {
      ...
      if (res?.data !== "") {
        setStateValues({
          gtstatemastid: res.data[0].gtstatemastid,
          ...
          {!fetchError ? (
            <>
              <div style={{ display: `${userRights[0].readonlys === "T" ? "block" : "none"}` }}>
                <ActionButtton
                  news={StateMaster_New}
                  saves={StateMaster_Save}
                  deletes={StateMaster_Delete}
                  searches={StateMaster_New}
                  prints={StateMaster_New}
                  treebutton={StateMaster_New}
                  globalsearch={StateMaster_New}
                  login={StateMaster_New}
                  changepassword={StateMaster_New}
                  changeskin={StateMaster_New}
                  contact={StateMaster_New}
                  pdf={StateMaster_New}
                  imports={StateMaster_New}
                  download={StateMaster_New}
                    ...
                    <div className="content active-content">
                      ...
                      <fieldset>
                        ...
                        <div className="row">
                          ...
                          <input className="col-md-4" type="text" name="statename" value={stateValues.statename || ""} onChange={handleChange} />
                        </div>
