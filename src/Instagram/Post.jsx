...
const navigate=useNavigate();
 ...
 const deleteHandler=async(deletedid)=>{
  await axios.delete(`${insert}/${deletedid}`).then((res) => {  
    if(res.data > 0){
     ...
     const likeorDislikeHandler=async(likeid)=>{
      ...
      try{
        ...
        eventname:'likes',       
         ...
         {          
            const updatedPostData=posts.map(p=>p.asptblinstapostid === post.asptblinstapostid ? {
              ...p, likes:liked ? p.likes.filter(asptblinstapostid=>asptblinstapostid !== post.asptblinstapostid) :
               [...p.likes,post.asptblinstapostid]
              } : p )
    ...
    <div className='container-fluid shadow' >
        ...
        <div key={post.asptblinstapostid} className="row"  >
                   <div className="d-flex   pe-3 ">
                    ...
                    <div className="p-1" > <Avatar  src={user?.profilePicture} alt={post.author}  > </Avatar></div>
                    <div className="p-1" >{post?.username} {post?.asptblinstapostid} {post?.id} </div>
                    ...
                    <div className="ms-auto p-1"  onClick={()=>handleChildShow(showChild)}> <BsThreeDots  />  </div>
                    <Modal show={showChild} onHide={()=>setShowChild(false)}>
                        <Modal.Header closeButton>
                         ...
                         <Modal.Body className='d-flex flex-column align-items-center  '>
                              ...
                              <Button onClick={()=>deleteHandler(post.asptblinstapostid)} variant='ghost' className='strong'>   Delete   s         </Button>
                                }
                          </Modal.Body>
                    </Modal>
                  </div>        
                  <p>{post.comments.user}</p>
                     ...
                     <div className="d-flex flex-row p-1">
                          <div className="w-100"  >
                            {
                              liked ? <FaHeart   onClick={()=>likeorDislikeHandler(post.asptblinstapostid)} size={'12'} style={{marginRight:'15px',color:"red"}}  /> : <FaRegHeart  size={'12'} onClick={()=>likeorDislikeHandler(post.asptblinstapostid)} style={{marginRight:'15px'}} />
                            }
                        <i className='bi bi-chat pe-2' onClick={()=>{
                    ...
                    <span className='pe-3' style={{color:"black", fontWeight:"bold"}} >{postLike} Likes</span>
                      <span className="d-flex text-truncate"  >{post.user?.username} {post.caption}</span>
                    {
                            comment && (
                                  <span className='pe-1' onClick={()=> { 
                                      dispatch(setSelectedPost(post));                                     
                                      setOpen(true)}}>
                           View {Number(comment.length)} Comments</span>  
                          )
                     }  
                 <CommentDialog key={post.asptblinstapostid} open={open} setOpen={setOpen} post={post} />
                  <div className='row p-2' >
                    ...
                    <textarea cols={'3'} rows={'3'}  placeholder='add comments..' value={text} onChange={TextChangeHandler} style={{border:"1px solid whitesmoke",padding:"2px"}} className='col-md-9'></textarea>
                        {text &&  <button type='button' onClick={()=>postHandler(post.asptblinstapostid)} className='btn col-md-1'   >post</button> }
                    </div>     
