...
const TreeViewMaster = ({ title, subTitle }) => {
  ...
  const deleteparam = API_URL + "/UserRights";
  ...
  useEffect(() => {
    async function fetchApi() {
      try {
        const [rightsRes, naviRes, menuRes, compRes] = await Promise.all([
  ...
  const TreeViewCheck1 = (id) => {
    try {
      ...
      if (id.userrightsid === 0) {
        setNaviItems1((prev) => prev.filter((item) => item.menuid !== id.menuid));
        return;
      }
  ...
  const TreeViewCheck = (id) => {
    try {
      ...
      // find item directly
      const selectedItem = navi_Items.find((post) => post.menuid === id.menuid);

      if (!selectedItem) return; // safety check

      // check if already exists
      const alreadyExists = navi_Items1.some((post) => post.menuid === id.menuid);
  ...
  const TreeView_Save = async () => {
    try {
      ...
      const users = navi_Items1.length === 0 ? navi_Items1 : ListData(navi_Items1);
      if (users.length === 0) {
  ...
  const TreeView_Delete = async () => {
    try {
      const users = naviDelete.length > 0 ? naviDelete : ListData(navi_Items1);
      ...
      const response = await axios.delete(insert_update, { data: users });
      toast.success(response?.data?.message);
      setNaviItems1([]);
    } catch (err) {
      console.error(err);
  ...
  const commentsData = useMemo(() => {
    ...
    // Filtering
    if (search) {
      computed = computed.filter((item) => item.menuname?.toLowerCase().includes(search) || item.navurl?.toLowerCase().includes(search));
    }
  ...
  const commentsData1 = useMemo(() => {
    ...
    if (sorting1.field) {
      ...
      computedComments1 = [...computedComments1].sort((a, b) => reversed * String(a[sorting1.field] || "").localeCompare(String(b[sorting1.field] || "")));
    }

    // Pagination
    return computedComments1.slice((currentPage1 - 1) * ITEM_PER_PAGE, (currentPage1 - 1) * ITEM_PER_PAGE + ITEM_PER_PAGE);
        ...
        <div className="container-fluid animate-zoom pt-0">
          <div className="row" style={{ display: `${userRights1[0].readonlys === "T" ? "block" : "none"}` }}>
            <ActionButtton
              news={TreeView_New}
              saves={TreeView_Save}
              deletes={TreeView_Delete}
              searches={TreeView_New}
              readonlys={TreeView_New}
              prints={TreeView_New}
              treebutton={TreeView_New}
              globalsearch={TreeView_New}
              login={TreeView_New}
              changepassword={TreeView_New}
              changeskin={TreeView_New}
              contact={TreeView_New}
              pdf={TreeView_New}
              imports={TreeView_New}
              download={TreeView_New}
            ...
            <div className="content-tabs p-1" style={{ display: "flex", justifyContent: "space-evenly" }}>
              <Search
                colorValue={colorValue}
                searchs={navi_naviSearch1}
                setsearchs={setNaviSearch1}
                  ...
                  {navi_Items1.length > 0 ? (
                    <>
                      <DataTable
                        ...
                        comments={navi_Items1}
                        setComments={setNaviItems1}
                        foreValue={foreValue}
                        searches={navi_naviSearch1}
                        setSearches={setNaviSearch1}
                        ...
                        ITEM_PER_PAGE={ITEM_PER_PAGE}
                        EditData={TreeViewCheck1}
                        commentsData={commentsData1}
                        checkall={checkall1}
                        setCheckAll={setCheckAll1}
                        checkchild={checkchild1}
                        setCheckchild={setCheckchild1}
                      />
                    </>
                  ) : (
                    <p style={{ marginTop: "2rem", color: "var(--bs-danger)" }}></p>
