...
const NavigationMaster = ({ title, subTitle }) => {
  ...
  useEffect(() => {
    const fetchAPI = async () => {
      try {
        const [rightsRes, naviRes, menuRes, compRes, userRes] = await Promise.all([
  ...
  useEffect(() => {
    ...
    const filterResult = navi_Items.filter((item) => item.menuname?.toLowerCase().includes(search));
  ...
  const Navigation_New = () => {
    setNaviValues([]);
    setNaviActive(false);
    axios
      .get(`${naviparam}/${defaultDetails.Compcode}`)
      .then((res) => {
        setNaviItems(res.data);
      })
      .catch((error) => {
  ...
  const commentsData = useMemo(() => {
    ...
    if (sorting.field) {
      ...
      computedComments = [...computedComments].sort((a, b) => reversed * String(a[sorting.field] || "").localeCompare(String(b[sorting.field] || "")));
    }
        ...
        <div className="container-fluid animate-zoom pt-0">
          <div className="row" style={{ display: `${userRights1[0].readonlys === "T" ? "block" : "none"}` }}>
            <ul className="boxShadow d-flex justify-content-end">
              <li>
                {" "}
                <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ display: `${userRights1[0].news}` === "T" ? "block" : "none", backgroundColor: `${colorValue}` }} onClick={() => Navigation_New()}>
                  News
                </button>
              </li>
              <li>
                {" "}
                <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ display: `${userRights1[0].saves}` === "T" ? "block" : "none", backgroundColor: `${colorValue}` }} onClick={() => Navigation_Save()}>
                  Save
                </button>
              </li>
              <li>
                {" "}
                <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ display: `${userRights1[0].deletes}` === "T" ? "block" : "none", backgroundColor: `${colorValue}` }} onClick={(e) => Navigation_Delete(naviValues.menuid)}>
                  Delete
                </button>
              </li>
              <li>
                {" "}
                <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ display: `${userRights1[0].search}` === "T" ? "block" : "none", backgroundColor: `${colorValue}` }} onClick={() => Navigation_New()}>
                  {" "}
                  Search{" "}
                </button>
              </li>
              <li>
                {" "}
                <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ display: `${userRights1[0].prints}` === "T" ? "block" : "none", backgroundColor: `${colorValue}` }} onClick={() => Navigation_New()}>
                  Prints
                </button>
              </li>
              <li>
                {" "}
                <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ display: `${userRights1[0].readonlys}` === "T" ? "none" : "none", backgroundColor: `${colorValue}` }} onClick={() => Navigation_New()}>
                  Readonlys
                </button>
              </li>
              <li>
                {" "}
                <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ display: `${userRights1[0].treebutton}` === "T" ? "block" : "none", backgroundColor: `${colorValue}` }} onClick={(e) => Navigation_New()}>
                  TreeButton
                </button>
              </li>
              <li>
                {" "}
                <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ display: `${userRights1[0].globalsearch}` === "T" ? "block" : "none", backgroundColor: `${colorValue}` }} onClick={() => Navigation_New()}>
                  {" "}
                  Globalsearch{" "}
                </button>
              </li>
              <li>
                {" "}
                <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ display: `${userRights1[0].login}` === "T" ? "block" : "none", backgroundColor: `${colorValue}` }} onClick={() => Navigation_New()}>
                  Login
                </button>
              </li>
              <li>
                {" "}
                <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ display: `${userRights1[0].changepassword}` === "T" ? "block" : "none", backgroundColor: `${colorValue}` }} onClick={() => Navigation_New()}>
                  Changepassword
                </button>
              </li>
              <li>
                {" "}
                <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ display: `${userRights1[0].changeskin}` === "T" ? "block" : "none", backgroundColor: `${colorValue}` }} onClick={(e) => Navigation_New()}>
                  Changeskin
                </button>
              </li>
              <li>
                {" "}
                <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ display: `${userRights1[0].contact}` === "T" ? "block" : "none", backgroundColor: `${colorValue}` }} onClick={() => Navigation_New()}>
                  {" "}
                  Contact{" "}
                </button>
              </li>
              <li>
                {" "}
                <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ display: `${userRights1[0].pdf}` === "T" ? "block" : "none", backgroundColor: `${colorValue}` }} onClick={(e) => Navigation_New()}>
                  Pdf
                </button>
              </li>
              <li>
                {" "}
                <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ display: `${userRights1[0].imports}` === "T" ? "block" : "none", backgroundColor: `${colorValue}` }} onClick={() => Navigation_New()}>
                  {" "}
                  Import{" "}
                </button>
              </li>
              <li>
                {" "}
                <button className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ display: `${userRights1[0].download}` === "T" ? "block" : "none", backgroundColor: `${colorValue}` }} onClick={() => Navigation_New()}>
                  {" "}
                  Download{" "}
                </button>
              </li>
              ...
              <div className="col-md-4 ">
                ...
                <div className="content active-content">
                  <div className="row py-1">
                    <label className="col-md-4" id="ID">
                      ID
                    </label>
                  ...
                  <div className="row py-1">
                    ...
                    <input className="col-md-7" type="text" name="menuname" value={naviValues.menuname || ""} onChange={handleChange} />
                  </div>
                  <div className="row">
                    <label className="col-md-4" htmlFor="">
                      Navigate Url
                    </label>
                    <input className="col-md-7" type="text" name="navurl" autoComplete="off" ref={inputref} value={naviValues.navurl || ""} onChange={handleChange} />
                  </div>
                  <div className="row py-1">
                    <label className="col-md-4">ParentID</label>
                    <input className="col-md-7" type="text" name="parentmenuid" autoComplete="off" ref={inputref} value={naviValues.parentmenuid || ""} onChange={handleChange} />
                  </div>
                  ...
                  {!fetchError && navi_Items.length > 0 ? (
                    <div className="pt-2 ">
                      <DataTable
                        ...
                        comments={navi_Items}
                        setComments={setNaviItems}
                        foreValue={foreValue}
                        searches={navi_naviSearch}
                        setSearches={setNaviSearch}
