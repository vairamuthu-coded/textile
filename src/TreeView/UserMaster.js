...
const UserMaster = ({ title, subTitle }) => {
  ...
  useEffect(() => {
    async function fetchAPI() {
      await axios
        ...
        .catch((error) => {
          setFetchError("a4" + error);
        });
      await axios
        .get(`${API_URL_username}/${defaultDetails.Compcode}`)
        .then((res) => {
    ...
    try {
      await axios.get(`${API_URL_username}/${id.compcode}/${id.userid}`).then((u) => {
        setUserValues({
          ...
          finyear: u.data[0].finyear,
          dept: u.data[0].dept,
          compcode: u.data[0].gtcompmastid,
          username: u.data[0].username,
          password: u.data[0].password,
          binarypassword: u.data[0].newpassword,
          sessiontime: u.data[0].sessiontime,
          active: u.data[0].active === "T" ? true : false,
        });
      });
    } catch (e) {
    } finally {
  ...
  const UserMaster_Delete = async (id) => {
    try {
      ...
      const errMsg = error.response?.data || error.message || "An error occurred while deleting";
            ...
            <>
              <div className="row" style={{ display: `${userRights1[0].readonlys === "T" ? "block" : "none"}` }}>
                <ul className="boxShadow d-flex justify-content-end">
                  <li>
                    {" "}
                    <button onClick={() => UserMaster_New()} className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ backgroundColor: `${colorValue}`, display: userRights1[0].news === "T" ? "block" : "none" }}>
                      New
                    </button>
                  </li>
                  <li>
                    {" "}
                    <button onClick={() => UserMaster_Save()} className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ backgroundColor: `${colorValue}`, display: userRights1[0].saves === "T" ? "block" : "none" }}>
                      Save
                    </button>
                  </li>
                  <li>
                    {" "}
                    <button onClick={() => UserMaster_Delete(userValues.userid)} className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ backgroundColor: `${colorValue}`, display: userRights1[0].deletes === "T" ? "block" : "none" }}>
                      Delete
                    </button>
                  </li>
                  <li>
                    {" "}
                    <button onClick={() => UserMaster_Search()} className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ backgroundColor: `${colorValue}`, display: userRights1[0].search === "T" ? "block" : "none" }}>
                      {" "}
                      Search{" "}
                    </button>
                  </li>
                  <li>
                    {" "}
                    <button onClick={() => UserMaster_Prints()} className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ backgroundColor: `${colorValue}`, display: userRights1[0].prints === "T" ? "block" : "none" }}>
                      Print
                    </button>
                  </li>
                  <li>
                    {" "}
                    <button onClick={() => UserMaster_New()} className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ backgroundColor: `${colorValue}`, display: userRights1[0].readonlys === "T" ? "none" : "none" }}>
                      Readonlys
                    </button>
                  </li>
                  <li>
                    {" "}
                    <button onClick={() => UserMaster_New()} className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ backgroundColor: `${colorValue}`, display: userRights1[0].treebutton === "T" ? "block" : "none" }}>
                      treebutton
                    </button>
                  </li>
                  <li>
                    {" "}
                    <button onClick={() => UserMaster_New()} className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ backgroundColor: `${colorValue}`, display: userRights1[0].globalsearch === "T" ? "block" : "none" }}>
                      {" "}
                      globalsearch{" "}
                    </button>
                  </li>
                  <li>
                    {" "}
                    <button onClick={() => UserMaster_New()} className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ backgroundColor: `${colorValue}`, display: userRights1[0].login === "T" ? "block" : "none" }}>
                      login
                    </button>
                  </li>
                  <li>
                    {" "}
                    <button onClick={() => UserMaster_New()} className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ backgroundColor: `${colorValue}`, display: userRights1[0].changepassword === "T" ? "block" : "none" }}>
                      changepassword
                    </button>
                  </li>
                  <li>
                    {" "}
                    <button onClick={() => UserMaster_New()} className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ backgroundColor: `${colorValue}`, display: userRights1[0].changeskin === "T" ? "block" : "none" }}>
                      changeskin
                    </button>
                  </li>
                  <li>
                    {" "}
                    <button onClick={() => UserMaster_New()} className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ backgroundColor: `${colorValue}`, display: userRights1[0].contact === "T" ? "block" : "none" }}>
                      {" "}
                      contact{" "}
                    </button>
                  </li>
                  <li>
                    {" "}
                    <button onClick={() => UserMaster_New()} className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ backgroundColor: `${colorValue}`, display: userRights1[0].pdf === "T" ? "block" : "none" }}>
                      pdf
                    </button>
                  </li>
                  <li>
                    {" "}
                    <button onClick={() => UserMaster_New()} className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ backgroundColor: `${colorValue}`, display: userRights1[0].imports === "T" ? "block" : "none" }}>
                      {" "}
                      import{" "}
                    </button>
                  </li>
                  <li>
                    {" "}
                    <button onClick={() => UserMaster_Prints()} className={newButton === 1 ? "tabs active-tabs" : "tabs"} style={{ backgroundColor: `${colorValue}`, display: userRights1[0].download === "T" ? "block" : "none", width: "100%" }}>
                      {" "}
                      Download{" "}
                    </button>
                  </li>
                  ...
                  <div className={newButton === 1 ? "content active-content" : "content"}>
                    <div className="content active-content">
                      ...
                      <div className="row py-1">
                        ...
                        <input className="col-md-3" type="text" name="binarypassword" ref={inputref} value={userValues.binarypassword || ""} onChange={handleChange} />

                        <Label className={`col-md-2`} labelName={"Password"}></Label>
                        <input className="col-md-3" type="text" name="password" ref={inputref} value={userValues.password || ""} onChange={handleChange} />
                      </div>
                      <div className="row">
                        <Label className={`col-md-2`} labelName={"SessionTime"}></Label>
                        <input className="col-md-3" type="text" name="sessiontime" ref={inputref} value={userValues.sessiontime || ""} onChange={handleChange} />
                    ...
                    <Search
                      ...
                      searchs={userMaster_Search}
                      setsearchs={setUserMaster_Search}
                    ...
                    {user_userItems.length > 0 ? (
                      <div className="pt-2">
                        <DataTable
                          ...
                          comments={user_userItems}
                          setComments={setUserItems}
                          foreValue={foreValue}
                          searches={userMaster_Search}
                          setSearches={setUserMaster_Search}
                      ...
                      <>
                        ...
                        >
                          ...
                          <Worker workerUrl="https://unpkg.com/pdfjs-dist@3.11.174/build/pdf.worker.min.js">
                            <Viewer theme="light" fileUrl={pdfFile} viewMode={ViewMode.SinglePage} plugins={[defaultLayoutPluginInstance]} defaultScale={SpecialZoomLevel.ActualSize} />
                          </Worker>
                        </div>
                        ;
                      </>
